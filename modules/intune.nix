...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    # Disable kwallet only when plasma6 is present
    security.pam.services.login.kwallet.enable =
      lib.mkIf config.services.desktopManager.plasma6.enable (lib.mkForce false);
    security.pam.services.sddm.kwallet.enable =
      lib.mkIf config.services.desktopManager.plasma6.enable (lib.mkForce false);

    # Route Edge secret storage through gnome-keyring
    environment.etc."opt/microsoft/msedge/msedge.conf".text = ''
